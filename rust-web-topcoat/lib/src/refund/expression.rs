...
impl<'a> RefundExpression<'a> {
    ...
    pub fn get_processed_date(self) -> crate::ValueExpression<'a, teaql_core::time::Timestamp> {
        let next = self.result.and_then("processed_date", |entity| entity.eval_processed_date());
        crate::ValueExpression::new(next, self.root_desc.clone())
    }
