...
#[teaql(entity = "ProductReview", table = "product_review_data", data_service = "sqlite")]
pub struct ProductReview {
#[teaql(id)]
    id: u64,

// @source massive_erp_model.xml:222
...
impl ProductReview {
    ...
    pub fn entity_key(&self) -> teaql_runtime::EntityKey {
        teaql_runtime::EntityKey::new("ProductReview", self.id)
    }
    ...
    pub fn review_text(&self) -> String {
        self.changed_review_text().and_then(|value| value.try_text().map(|value| value.to_owned())).unwrap_or_else(|| self.review_text.clone())
    }

    pub fn update_review_text(&mut self, value: impl Into<teaql_core::Value>) -> &mut Self {
        let value = value.into();
        self.review_text = value.try_text().map(|value| value.trim().to_owned()).unwrap_or_else(|| self.review_text.clone());
        self.root.set(self.entity_key(), "review_text", value);
        self
    }

    pub fn changed_review_text(&self) -> Option<teaql_core::Value> {
        self.root.get(&self.entity_key(), "review_text")
    }

    pub fn eval_review_text(&self) -> teaql_core::eval::EvalResult<String> {
        if !self.is_loaded("review_text") {
                    teaql_core::eval::EvalResult::NotLoaded { failed_node: "review_text".to_string(), attempted_path: "review_text".to_string() }
                } else {
                    teaql_core::eval::EvalResult::Value(self.review_text())
                }}
