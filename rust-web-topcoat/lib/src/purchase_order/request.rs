...
impl EntityReference for crate::PurchaseOrder {
...
#[derive(Debug)]
pub struct PurchaseOrderRequest<R = crate::PurchaseOrder> {
    query: SelectQuery,
...
impl<R> Clone for PurchaseOrderRequest<R> {
    fn clone(&self) -> Self {
        Self {
...
impl<R> PurchaseOrderRequest<R> {
    pub(crate) fn new() -> Self {
        Self {
            query: SelectQuery::new("PurchaseOrder")
                .project("id")
                .project("version"),
    ...
    pub fn return_type<T>(self) -> PurchaseOrderRequest<T> {
    ...
    pub(crate) async fn _execute_for_list<'a, C>(
        self,
        ctx: &'a C,
    ) -> Result<SmartList<R>, TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
    ...
    pub(crate) async fn _execute_for_stream<'a, C>(
        self,
        ctx: &'a C,
    ) -> Result<Vec<teaql_data_service::StreamChunk>, TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: TeaqlRepositoryProvider + ?Sized,
    {
        let repository = ctx
    ...
    pub(crate) async fn _execute_for_first<'a, C>(
        self,
        ctx: &'a C,
    ) -> Result<Option<R>, TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: TeaqlRepositoryProvider + ?Sized,
        R: teaql_core::Entity,
    {
    ...
    pub(crate) async fn _execute_for_count<'a, C>(
        self,
        ctx: &'a C,
    ) -> Result<u64, TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: TeaqlRepositoryProvider + ?Sized,
    {
        let repository = ctx
        ...
        rows.first()
            ...
            .ok_or_else(|| DataServiceError::Runtime(RuntimeError::Graph(format!("count result for PurchaseOrder is missing or not numeric"))))
    }

    pub(crate) async fn _execute_for_exists<'a, C>(
        self,
        ctx: &'a C,
    ) -> Result<bool, TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: TeaqlRepositoryProvider + ?Sized,
    {
        let repository = ctx
    ...
    pub(crate) async fn _execute_for_records<'a, C>(
        self,
        ctx: &'a C,
    ) -> Result<SmartList<Record>, TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: TeaqlRepositoryProvider + ?Sized,
    {
        let repository = ctx
    ...
    pub(crate) async fn _execute_for_record<'a, C>(
        self,
        ctx: &'a C,
    ) -> Result<Option<Record>, TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: TeaqlRepositoryProvider + ?Sized,
    {
    ...
    fn dynamic_json_self_field(field: &str) -> Option<&'static str> {
        match field {
            ...
            "supplier" | "supplier_id" => Some("supplier_id"),
            _ => None,
        }
    }
    ...
    pub fn select_self(mut self) -> Self {
        ...
        self.query = self.query.project("supplier_id");
        self
    }
    ...
    pub fn select_all(self) -> Self {
        ...
        request = request.select_supplier();
        request
    }
    ...
    pub fn filter_by_supplier(mut self, value: impl EntityReference) -> Self {
        self.query = self.query.and_filter(Expr::eq("supplier_id", value.entity_id_value()));
        self
    }

    pub fn with_supplier_matching(mut self, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
        ...
        self.relation_filters.push(RelationFilter::new("supplier", selection));
        self
    }


    pub fn without_supplier_matching(mut self, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
    ...
    pub fn have_supplier(mut self) -> Self {
        self.query = self.query.and_filter(Expr::is_not_null("supplier_id"));
        self
    }

    pub fn have_no_supplier(mut self) -> Self {
        self.query = self.query.and_filter(Expr::is_null("supplier_id"));
        self
    }


    pub fn group_by_supplier(self) -> Self {
        self.group_by("supplier_id")
    }

    pub fn group_by_supplier_as(self, alias: impl Into<String>) -> Self {
        let alias = alias.into();
        let mut request = self.group_by("supplier_id");
        request.query = request
            .query
            .project_expr(alias, Expr::column("supplier_id"));
        request
    }

    pub fn group_by_supplier_with_function(
        self,
        alias: impl Into<String>,
        function: AggregateFunction,
    ) -> Self {
        self.group_by("supplier_id")
            .aggregate_with_function("supplier_id", alias, function)
    }

    pub fn group_by_supplier_with(mut self, request: impl Into<QuerySelection>) -> Self {
        self.query = self.query.group_by("supplier_id");
    ...
    pub fn group_by_supplier_with_details(self) -> Self {
        self.group_by_supplier_with_details_from(crate::Q::suppliers().unlimited())
    }

    pub fn group_by_supplier_with_details_from(self, request: impl Into<QuerySelection>) -> Self {
        self.group_by_supplier_with(request)
    }


    pub fn roll_up_to_supplier(self) -> Self {
        self.roll_up_to_supplier_with(crate::Q::suppliers().unlimited())
    }

    pub fn roll_up_to_supplier_with(self, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
        self.with_supplier_matching(selection.clone())
            .group_by_supplier_with(selection)
    }

    pub fn count_supplier(self) -> Self {
        self.count_supplier_as("supplier_count")
    }

    pub fn count_supplier_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_count_field("supplier_id", alias)
    }

    pub fn unselect_supplier(mut self) -> Self {
        self.query.projection.retain(|field| field != "supplier_id");
        self.query.relations.retain(|relation| relation.name != "supplier");
        self
    }
    pub fn select_supplier(mut self) -> Self {
        self.query = self.query.relation("supplier");
        self
    }

    pub fn select_supplier_with(mut self, request: impl Into<QuerySelection>) -> Self {
        let selection = request.into();
        self.query = self.query.relation_query("supplier", selection.clone().into_query());
        self.relation_selections.push(RelationSelection::new("supplier", selection));
        self
}

    pub fn facet_by_supplier_as(self, facet_name: impl Into<String>, request: impl Into<QuerySelection>) -> Self {
        self.facet_by_supplier_as_with_options(facet_name, request, true)
    }

    pub fn facet_by_supplier_as_with_options(
        mut self,
...
impl<R> Default for PurchaseOrderRequest<R> {
    fn default() -> Self {
        Self::new()
    }
}

impl<R> From< PurchaseOrderRequest<R> > for SelectQuery {
    fn from(request: PurchaseOrderRequest<R>) -> Self {
        QuerySelection::from(request).into_query()
    }
}

impl<R> From< PurchaseOrderRequest<R> > for QuerySelection {
    fn from(request: PurchaseOrderRequest<R>) -> Self {
        Self {
            query: request.query,
...
impl<'a, C> crate::request_support::AuditedSave<'a, C> for teaql_core::Audited<crate::PurchaseOrder> 
where C: crate::request_support::TeaqlRepositoryProvider + ?Sized + 'a
{
    type Error = crate::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>;
...
impl<R: teaql_core::Entity> crate::PurposedQuery<PurchaseOrderRequest<R>> {
    pub fn new_entity<C>(&self, ctx: &C) -> crate::PurchaseOrder
    where
        C: crate::TeaqlRuntime + ?Sized,
    {
        crate::PurchaseOrder::runtime_new(ctx.user_context().entity_root())
    }

    fn into_inner_with_trace(mut self) -> PurchaseOrderRequest<R> {
    ...
    pub async fn execute_for_page<'a, C>(
        ...
        offset: u64,
        limit: u64,
    ) -> Result<teaql_core::SmartList<R>, crate::request_support::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
    ...
    pub async fn execute_for_exists<'a, C>(
        self,
        ctx: &'a C,
    ) -> Result<bool, crate::request_support::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: crate::request_support::TeaqlRepositoryProvider + ?Sized,
    {
    ...
    pub async fn execute_for_list<'a, C>(self, ctx: &'a C) -> Result<teaql_core::SmartList<R>, crate::request_support::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: crate::request_support::TeaqlRepositoryProvider + ?Sized,
    {
    ...
    pub async fn execute_for_stream<'a, C>(self, ctx: &'a C) -> Result<Vec<teaql_data_service::StreamChunk>, crate::request_support::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: crate::request_support::TeaqlRepositoryProvider + ?Sized,
    {
    ...
    pub async fn execute_for_first<'a, C>(self, ctx: &'a C) -> Result<Option<R>, crate::request_support::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: crate::request_support::TeaqlRepositoryProvider + ?Sized,
    {
    ...
    pub async fn execute_for_one<'a, C>(self, ctx: &'a C) -> Result<Option<R>, crate::request_support::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: crate::request_support::TeaqlRepositoryProvider + ?Sized,
    {
    ...
    pub async fn execute_for_records<'a, C>(self, ctx: &'a C) -> Result<teaql_core::SmartList<teaql_core::Record>, crate::request_support::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: crate::request_support::TeaqlRepositoryProvider + ?Sized,
    {
    ...
    pub async fn execute_for_record<'a, C>(self, ctx: &'a C) -> Result<Option<teaql_core::Record>, crate::request_support::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: crate::request_support::TeaqlRepositoryProvider + ?Sized,
    {
    ...
    pub async fn execute_for_count<'a, C>(self, ctx: &'a C) -> Result<u64, crate::request_support::TeaqlDataServiceError<C::PurchaseOrderRepository<'a>>>
    where
        C: crate::request_support::TeaqlRepositoryProvider + ?Sized,
    {
