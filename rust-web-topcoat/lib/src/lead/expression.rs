...
impl<'a> LeadExpression<'a> {
    ...
    pub fn get_converted_customer_id(self) -> crate::ValueExpression<'a, String> {
        let next = self.result.and_then("converted_customer_id", |entity| entity.eval_converted_customer_id());
        crate::ValueExpression::new(next, self.root_desc.clone())
    }
    ...
    pub fn get_opportunity_list(self) -> crate::OpportunityListExpression<'a> {
        let next = self.result.and_then("opportunity_list", |entity| entity.eval_opportunity_list());
        crate::OpportunityListExpression::new(next, self.root_desc.clone())
    }
}
