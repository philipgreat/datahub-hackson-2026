...
impl<'a> ExpenseExpression<'a> {
    ...
    pub fn get_date_incurred(self) -> crate::ValueExpression<'a, teaql_core::time::Timestamp> {
        let next = self.result.and_then("date_incurred", |entity| entity.eval_date_incurred());
        crate::ValueExpression::new(next, self.root_desc.clone())
    }
