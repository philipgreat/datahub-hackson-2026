...
impl<'a> InvoiceExpression<'a> {
    ...
    pub fn get_paid_amount(self) -> crate::ValueExpression<'a, rust_decimal::Decimal> {
        let next = self.result.and_then("paid_amount", |entity| entity.eval_paid_amount());
        crate::ValueExpression::new(next, self.root_desc.clone())
    }

    pub fn get_is_paid(self) -> crate::ValueExpression<'a, bool> {
        let next = self.result.and_then("is_paid", |entity| entity.eval_is_paid());
        crate::ValueExpression::new(next, self.root_desc.clone())
    }
