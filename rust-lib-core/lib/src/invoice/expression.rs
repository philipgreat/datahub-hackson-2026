...
impl<'a> InvoiceExpression<'a> {
    ...
    pub fn get_due_date(self) -> crate::ValueExpression<'a, teaql_core::time::Timestamp> {
        let next = self.result.and_then("due_date", |entity| entity.eval_due_date());
        crate::ValueExpression::new(next, self.root_desc.clone())
    }

    pub fn get_issue_date(self) -> crate::ValueExpression<'a, teaql_core::time::Timestamp> {
