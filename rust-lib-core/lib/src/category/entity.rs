...
#[teaql(entity = "Category", table = "category_data", data_service = "sqlite")]
pub struct Category {
#[teaql(id)]
    id: u64,

// @source massive_erp_model.xml:201
    description: String,
...
#[teaql(version)]
    version: i64,
#[teaql(relation(target = "Product", local_key = "id", foreign_key = "category_id", many))]
...
impl Category {
    ...
    pub fn entity_key(&self) -> teaql_runtime::EntityKey {
        teaql_runtime::EntityKey::new("Category", self.id)
    }
    ...
    pub fn parent_id(&self) -> String {
        self.changed_parent_id().and_then(|value| value.try_text().map(|value| value.to_owned())).unwrap_or_else(|| self.parent_id.clone())
    }

    pub fn update_parent_id(&mut self, value: impl Into<teaql_core::Value>) -> &mut Self {
        let value = value.into();
        self.parent_id = value.try_text().map(|value| value.trim().to_owned()).unwrap_or_else(|| self.parent_id.clone());
        self.root.set(self.entity_key(), "parent_id", value);
        self
    }

    pub fn changed_parent_id(&self) -> Option<teaql_core::Value> {
        self.root.get(&self.entity_key(), "parent_id")
    }

    pub fn eval_parent_id(&self) -> teaql_core::eval::EvalResult<String> {
        if !self.is_loaded("parent_id") {
                    teaql_core::eval::EvalResult::NotLoaded { failed_node: "parent_id".to_string(), attempted_path: "parent_id".to_string() }
                } else {
                    teaql_core::eval::EvalResult::Value(self.parent_id())
                }}
    ...
    pub fn eval_product_list(&self) -> teaql_core::eval::EvalResult<&SmartList<crate::Product>> {
        if !self.is_loaded("product_list") {
            ...
            teaql_core::eval::EvalResult::Value(&self.product_list)
        }
    }
