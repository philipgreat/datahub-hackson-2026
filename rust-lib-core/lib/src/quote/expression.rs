...
impl<'a> QuoteExpression<'a> {
    ...
    pub fn get_valid_until(self) -> crate::ValueExpression<'a, teaql_core::time::Timestamp> {
    ...
    pub fn get_opportunity_id(self) -> crate::ValueExpression<'a, u64> {
        let next = self.result.and_then("opportunity_id", |entity| entity.eval_opportunity_id());
        crate::ValueExpression::new(next, self.root_desc.clone())
    }
    pub fn get_opportunity(self) -> crate::OpportunityExpression<'a> {
        let next = self.result.and_then("opportunity", |entity| entity.eval_opportunity());
