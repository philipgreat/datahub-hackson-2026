...
impl<R> PayrollRequest<R> {
    ...
    pub fn select_self(mut self) -> Self {
        ...
        self.query = self.query.project("tax");
    ...
    pub fn select_tax(mut self) -> Self {
        self.query = self.query.project("tax");
        self
    }

    pub fn project_tax(self) -> Self {
        self.select_tax()
    }

    pub fn select_tax_raw(self, raw_sql_segment: impl Into<String>) -> Self {
        self.select_tax_unsafe_raw(UnsafeRawSqlSegment::trusted(raw_sql_segment))
    }

    pub fn select_tax_unsafe_raw(mut self, raw_sql_segment: UnsafeRawSqlSegment) -> Self {
        self.query_options
            .raw_projections
            .push(RawProjection::new("tax", raw_sql_segment));
        self
    }

    pub fn select_tax_with_function(self, function: AggregateFunction) -> Self {
        self.select_tax_as_with_function("tax", function)
    }

    pub fn select_tax_as_with_function(
        self,
        alias: impl Into<String>,
        function: AggregateFunction,
    ) -> Self {
        self.aggregate_with_function("tax", alias, function)
    }

    pub fn group_by_tax(self) -> Self {
        self.group_by("tax")
    }

    pub fn group_by_tax_as(self, alias: impl Into<String>) -> Self {
        let alias = alias.into();
        let mut request = self.group_by("tax");
        request.query = request
            .query
            .project_expr(alias, Expr::column("tax"));
        request
    }

    pub fn group_by_tax_with_function(
        self,
        alias: impl Into<String>,
        function: AggregateFunction,
    ) -> Self {
        self.group_by("tax")
            .aggregate_with_function("tax", alias, function)
    }

    pub fn count_tax(self) -> Self {
        self.count_tax_as("tax_count")
    }

    pub fn count_tax_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_count_field("tax", alias)
    }

    pub fn sum_tax(self) -> Self {
        self.sum_tax_as("sum_tax")
    }

    pub fn sum_tax_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_sum("tax", alias)
    }

    pub fn avg_tax(self) -> Self {
        self.avg_tax_as("avg_tax")
    }

    pub fn avg_tax_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_avg("tax", alias)
    }

    pub fn min_tax(self) -> Self {
        self.min_tax_as("min_tax")
    }

    pub fn min_tax_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_min("tax", alias)
    }

    pub fn max_tax(self) -> Self {
        self.max_tax_as("max_tax")
    }

    pub fn max_tax_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_max("tax", alias)
    }

    pub fn standard_deviation_tax(self) -> Self {
        self.standard_deviation_tax_as("stdDev_tax")
    }

    pub fn standard_deviation_tax_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_stddev("tax", alias)
    }

    pub fn square_root_of_population_standard_deviation_tax(self) -> Self {
        self.square_root_of_population_standard_deviation_tax_as("stdDevPop_tax")
    }

    pub fn square_root_of_population_standard_deviation_tax_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_stddev_pop("tax", alias)
    }

    pub fn sample_variance_tax(self) -> Self {
        self.sample_variance_tax_as("varSamp_tax")
    }

    pub fn sample_variance_tax_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_var_samp("tax", alias)
    }

    pub fn sample_population_variance_tax(self) -> Self {
        self.sample_population_variance_tax_as("varPop_tax")
    }

    pub fn sample_population_variance_tax_as(self, alias: impl Into<String>) -> Self {
        self.aggregate_var_pop("tax", alias)
    }

    pub fn unselect_tax(mut self) -> Self {
        self.query.projection.retain(|field| field != "tax");
        self.query_options.raw_projections.retain(|projection| projection.property_name != "tax");
        self
    }
    ...
    pub fn with_tax_is(mut self, value: impl Into<teaql_core::Value>) -> Self {
        self.query = self.query.and_filter(Expr::eq("tax", value));
        self
    }



    pub fn with_tax_is_not(mut self, value: impl Into<teaql_core::Value>) -> Self {
        self.query = self.query.and_filter(Expr::ne("tax", value));
        self
    }

    pub fn with_tax_greater_than(mut self, value: impl Into<teaql_core::Value>) -> Self {
        self.query = self.query.and_filter(Expr::gt("tax", value));
        self
    }

    pub fn with_tax_greater_than_or_equal_to(mut self, value: impl Into<teaql_core::Value>) -> Self {
        self.query = self.query.and_filter(Expr::gte("tax", value));
        self
    }

    pub fn with_tax_less_than(mut self, value: impl Into<teaql_core::Value>) -> Self {
        self.query = self.query.and_filter(Expr::lt("tax", value));
        self
    }

    pub fn with_tax_less_than_or_equal_to(mut self, value: impl Into<teaql_core::Value>) -> Self {
        self.query = self.query.and_filter(Expr::lte("tax", value));
        self
    }

    pub fn with_tax_between(
        mut self,
        lower: impl Into<teaql_core::Value>,
        upper: impl Into<teaql_core::Value>,
    ) -> Self {
        self.query = self.query.and_filter(Expr::between("tax", lower, upper));
        self
    }

    pub fn with_tax_between_range<T>(mut self, range: DateRange<T>) -> Self
    where
    ...
    pub fn with_tax_before(mut self, value: impl Into<teaql_core::Value>) -> Self {
        self.query = self.query.and_filter(Expr::lt("tax", value));
        self
    }

    pub fn with_tax_after(mut self, value: impl Into<teaql_core::Value>) -> Self {
        self.query = self.query.and_filter(Expr::gt("tax", value));
        self
    }

    pub fn with_tax_is_unknown(mut self) -> Self {
        self.query = self.query.and_filter(Expr::is_null("tax"));
        self
    }



    pub fn with_tax_is_known(mut self) -> Self {
        self.query = self.query.and_filter(Expr::is_not_null("tax"));
        self
    }


    pub fn order_by_tax_asc(mut self) -> Self {
        self.query = self.query.order_asc("tax");
        self
    }

    pub fn order_by_tax_desc(mut self) -> Self {
        self.query = self.query.order_desc("tax");
        self
    }

    pub fn order_by_tax_asc_using_gbk(mut self) -> Self {
        self.query = self.query.order_gbk_asc("tax");
        self
    }

    pub fn order_by_tax_desc_using_gbk(mut self) -> Self {
        self.query = self.query.order_gbk_desc("tax");
        self
    }
