...
impl<'a> InventoryItemExpression<'a> {
    ...
    pub fn get_quantity_on_hand(self) -> crate::ValueExpression<'a, rust_decimal::Decimal> {
        let next = self.result.and_then("quantity_on_hand", |entity| entity.eval_quantity_on_hand());
        crate::ValueExpression::new(next, self.root_desc.clone())
    }

    pub fn get_reorder_level(self) -> crate::ValueExpression<'a, rust_decimal::Decimal> {
