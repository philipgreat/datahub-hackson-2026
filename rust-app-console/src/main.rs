...
pub fn process_stream(transactions: Vec<serde_json::Value>) -> usize {
    ...
    for tx in transactions {
        ...
        if amount <= 0.0 {
            println!("🚨 过滤脏数据 (Data Quality Constraint Failed): {}", tx);
            continue;
        }
        println!("✅ 处理有效交易，脱敏账户信息: {}", "*****");
        valid_count += 1;
    }
    valid_count
}
