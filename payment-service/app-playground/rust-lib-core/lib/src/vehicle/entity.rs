...
#[teaql(relation(target = "MoveOrder", local_key = "id", foreign_key = "vehicle_id", many))]
...
impl Vehicle {
    ...
    pub fn plate_number(&self) -> String {
        self.changed_plate_number().and_then(|value| value.try_text().map(|value| value.to_owned())).unwrap_or_else(|| self.plate_number.clone())
    }

    pub fn update_plate_number(&mut self, value: impl Into<teaql_core::Value>) -> &mut Self {
        let value = value.into();
        self.plate_number = value.try_text().map(|value| value.trim().to_owned()).unwrap_or_else(|| self.plate_number.clone());
        self.root.set(self.entity_key(), "plate_number", value);
        self
    }

    pub fn changed_plate_number(&self) -> Option<teaql_core::Value> {
        self.root.get(&self.entity_key(), "plate_number")
    }

    pub fn eval_plate_number(&self) -> teaql_core::eval::EvalResult<String> {
        if !self.is_loaded("plate_number") {
                    teaql_core::eval::EvalResult::NotLoaded { failed_node: "plate_number".to_string(), attempted_path: "plate_number".to_string() }
                } else {
                    teaql_core::eval::EvalResult::Value(self.plate_number())
                }}
    ...
    pub fn capacity_cubic_feet(&self) -> i64 {
        self.changed_capacity_cubic_feet().and_then(|value| value.try_i64()).map(|value| value as i64).unwrap_or(self.capacity_cubic_feet)
    }

    pub fn update_capacity_cubic_feet(&mut self, value: impl Into<teaql_core::Value>) -> &mut Self {
        let value = value.into();
        self.capacity_cubic_feet = value.try_i64().map(|value| value as i64).unwrap_or(self.capacity_cubic_feet.clone());
        self.root.set(self.entity_key(), "capacity_cubic_feet", value);
        self
    }

    pub fn changed_capacity_cubic_feet(&self) -> Option<teaql_core::Value> {
        self.root.get(&self.entity_key(), "capacity_cubic_feet")
    }

    pub fn eval_capacity_cubic_feet(&self) -> teaql_core::eval::EvalResult<i64> {
        if !self.is_loaded("capacity_cubic_feet") {
                    teaql_core::eval::EvalResult::NotLoaded { failed_node: "capacity_cubic_feet".to_string(), attempted_path: "capacity_cubic_feet".to_string() }
                } else {
                    teaql_core::eval::EvalResult::Value(self.capacity_cubic_feet())
                }}
