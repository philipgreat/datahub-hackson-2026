...
#[teaql(entity = "Review", table = "review_data", data_service = "sqlite")]
pub struct Review {
#[teaql(id)]
    id: u64,
...
impl Review {
    ...
    pub fn entity_key(&self) -> teaql_runtime::EntityKey {
        teaql_runtime::EntityKey::new("Review", self.id)
    }
    ...
    pub fn rating(&self) -> i64 {
        self.changed_rating().and_then(|value| value.try_i64()).map(|value| value as i64).unwrap_or(self.rating)
    }

    pub fn update_rating(&mut self, value: impl Into<teaql_core::Value>) -> &mut Self {
        let value = value.into();
        self.rating = value.try_i64().map(|value| value as i64).unwrap_or(self.rating.clone());
    ...
    pub fn eval_rating(&self) -> teaql_core::eval::EvalResult<i64> {
